  ...
  - intros H1 H2.
           ...
           } ).
    {
      exists (fun w => match cov_dec C w with
               | inl proof => projT1 (H2 w proof)
               | inr nCw => fun _ => False
               end).

      split; intro w'; destruct (cov_dec C w') as [HCw' | HnCw']; intro abs; destruct (H2 w') as [Dw' [Hcov Hsat]]; auto.
      * apply HnCw' in abs; contradiction.
      * contradiction.
    }
